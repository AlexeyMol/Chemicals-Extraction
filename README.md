...
Библиотека может быть использована для решения прикладных задач:
 ...
 - Разработка специализированных поисковых машин.
...
- CPU не менее 8 ядер с частотой не менее 4Ггц
- RAM не менее 328 ГБ (от 64 Гб - но чем меньше время индексирования растёт экспоненциально)
- SSD nvme не менее 1.5 ТБ
- GPU уровня 4060TI 16GB или мощнее
...
```
Далее необходимо установить сопутствующие пакеты необходимые для установки различных компонентов системы:
```bash
...
# !!! Перед выполнением команд убедиться, что сервер СУБД и CLUSTER запущены.!!!
...
После того как развернули базу данных из бекапа необходимо ее проиндексировать (перед запуском убедитесь, что выполняете команды как пользователь postgres):
```sql
psql
\c chem_db_vector;
...
```
где:
- maintenance_work_mem - размер единичного графа для индексирования. Чем больше вы выставляете значение, тем больше будет требоваться памяти, но тем быстрее будет идти  индексация.
- max_parallel_maintenance_workers - Сколько потоков будут задействованы в индексировании + 1 главный.
...
# Функции
Извлекает названия химических структур из текста. Входные параметры - текст на русском или английском языке. Выходные параметры - массив токенов, характеризующих названия химических структур, и их координаты в тексте (начало и конец подстроки)
...
```
Получает список InChI для массива химических структур.
...
```
Стандартизирует в mol файл. Эта функция выполняет стандартизацию химических структур в MOL формат, при этом на вход ей подаются молекулы в форматах MOL, InChI, SMILES. Примеры использования ниже.
...
Извлечение названий химических структур, получение их InChI и их стандартизация.
...
if __name__ == "__main__":
    main()
```
Ожидаемый результат: набор стандартизованных mol файлов для всех извлеченных химических структур.
...
if __name__ == "__main__":
    main()
```
Ожидаемый результат: набор стандартизованный mol файл.
...
if __name__ == "__main__":
    main()
```
[Ожидаемый результат выполнения](examples/result.json)

Извлечение названий химических структур и сохранение результатов в json формате.
```python
from mol_analyzer  import ChemicalStructures as bt
...
Ожидаемый результат выполнения: Содержимое файла data.json
```json
{
...
# Благодарности
Библиотека создана при поддержке Фонда содействия инновациям (Договор №37ГУКодИИС12-D7/86408 о предоставлении гранта на выполнение проекта открытых библиотек от 09.08.2023).
